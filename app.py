...
async def generate_from_doc(request: Request, docFile: UploadFile = File(...), username: str = Form(...)):
    ip = request.client.host or "unknown"
    browser = request.headers.get("user-agent", "unknown")
    logs = load_logs()

    content = ""
    path = os.path.join(UPLOAD_FOLDER, f"{username}_{docFile.filename}")
    ...
    try:
        ...
        print(f"Processing file: {docFile.filename}")
        
        
        if docFile.filename.endswith(".docx"):
            try:
                doc = Document(path)
        ...
        elif docFile.filename.endswith(".txt"):
            try:
            ...
            raise HTTPException(status_code=400, detail="Unsupported file format. Please use .docx, .txt, or .pdf")
        ...
        ppt_path = create_enhanced_ppt(content, "", username)
        ...
        print(f"Unexpected error in generate_from_doc: {e}")
        import traceback
        traceback.print_exc()
